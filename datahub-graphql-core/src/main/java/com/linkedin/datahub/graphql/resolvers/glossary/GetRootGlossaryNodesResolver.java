...
public class GetRootGlossaryNodesResolver implements DataFetcher<CompletableFuture<GetRootGlossaryNodesResult>> {
  ...
  @Override
  public CompletableFuture<GetRootGlossaryNodesResult> get(final DataFetchingEnvironment environment) throws Exception {
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      try {
        final Filter filter = SearchUtils.getAuthorizedBaseFilter(context,buildGlossaryEntitiesFilter(), "GetRootGlossaryNodesResolver");
        final SearchResult gmsNodesResult = _entityClient.filter(
        ...
        final List<Urn> glossaryNodeUrns = gmsNodesResult.getEntities()
                .stream()
