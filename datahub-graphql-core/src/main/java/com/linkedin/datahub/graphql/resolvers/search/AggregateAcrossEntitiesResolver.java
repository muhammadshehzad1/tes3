...
/**
 * Executes a search query only to get a provided list of aggregations back.
 * Does not resolve any entities as results.
 */
@Slf4j
@RequiredArgsConstructor
public class AggregateAcrossEntitiesResolver implements DataFetcher<CompletableFuture<AggregateResults>> {
  ...
  public CompletableFuture<AggregateResults> get(DataFetchingEnvironment environment) {
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      final List<String> facets = input.getFacets() != null && input.getFacets().size() > 0 ? input.getFacets() : null;

      try {
        return mapAggregateResults(_entityClient.searchAcrossEntities(
        ...
        throw new RuntimeException(
            "Failed to execute aggregate across entities: " + String.format("entity types %s, query %s, filters: %s",
                input.getTypes(), input.getQuery(), input.getOrFilters()), e);
      }
    });
  }
  ...
  AggregateResults mapAggregateResults(SearchResult searchResult) {
    ...
    results.setFacets(searchResult.getMetadata().getAggregations().stream().map(MapperUtils::mapFacet).collect(Collectors.toList()));
